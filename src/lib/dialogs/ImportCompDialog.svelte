<script lang="ts">
  ...
  export const { onconfirm }: DialogExports = {
    async onconfirm() {
      ...
      const addRequest = idb.objectStore("comps", "readwrite").add($state.snapshot(importedComp));
      addRequest.onerror = () => {
        error = "Could not import comp";
      };

      addRequest.onsuccess = async () => {
        goto(`#/comp/${addRequest.result}`);
      };
    },
  };
