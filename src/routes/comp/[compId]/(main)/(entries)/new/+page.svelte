<script lang="ts">
  ...
  function getNewEntry():
    ...
    if (survey.type == "match") {
      ...
      const entries = data.entryRecords.filter((e) => e.surveyId == survey.id) as PitEntry[];
  ...
  const adjacentMatches = $derived.by(() => {
    ...
    const previous = data.matches.findLast((m) => compareMatches(m, match) < 0);
    const next = data.matches.find((m) => compareMatches(m, match) > 0);
  ...
  const matchingEntries = $derived.by(() => {
    ...
    return data.entryRecords.filter((e) => {
  ...
  const matchData = $derived(
    data.matches.find((m) => newEntry.type == "match" && compareMatches(m, newEntry.state.match) == 0),
  );
  ...
  function onconfirm() {
    ...
    if (newEntry.type == "match") {
      ...
      if ($scoutStore) {
        entry.scout = $scoutStore;
        if ($teamStore) {
          entry.scoutTeam = $teamStore;
        }
  ...
  function selectTargetTeamFromMatch(identifier: MatchIdentifier) {
    const match = data.matches.find((m) => compareMatches(m, identifier) == 0);
